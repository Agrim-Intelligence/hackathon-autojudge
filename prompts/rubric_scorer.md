...
- A live URL that is **reachable and rendered** (the Browser verifier reports `Reachable: True`, a real `page_title`, and `auth_blocked: False`) is **partial UX/Functional evidence even when the scripted journeys did not complete** within step budget. In that case score UX and Functional with a non-null `raw_score` and `evidence_kind: "inferred"` (the rendered UI was observed); reserve `raw_score: null` + `insufficient` for genuinely unreachable, credential-walled, or blank/non-rendering pages. Keep `verified` only for a successful browser journey. A render-only page (no journey completed) is deterministically capped at 7/10 on UX, so do not score it above that on render evidence alone.
